package com.gjp.facecamera_0401.arcsoft.faceserver;
...
import com.gjp.facecamera_0401.arcsoft.model.FaceRegisterInfo;
...
public class FaceServer {
    ...
    //图片保存目录
    public static final File SELF_DEFAULT_SAVE_FILE = new File(Environment.getExternalStorageDirectory(),
            Constants.SAVE_EXTRA_DIR);
    
    public File appRootDir = null;
    ...
    private FaceServer() {
        if (!SELF_DEFAULT_SAVE_FILE.exists()) {
            SELF_DEFAULT_SAVE_FILE.mkdirs();
        }
    }
    ...
    public File getAppRootDir() {
        if (appRootDir == null) {
            appRootDir = SELF_DEFAULT_SAVE_FILE;
        }
        return appRootDir;
    }
    ...
    public void initFaceList(Context context) {
        synchronized (this) {
            File featureDir = new File(getAppRootDir(), "features");
    ...
    public int getFaceNumber(Context context) {
        synchronized (this) {
            ...
            File featureFileDir = new File(getAppRootDir(), "features");
            int featureCount = 0;
            ...
            int imageCount = 0;
            File imgFileDir = new File(getAppRootDir(), "imgs");
     ...
     * @return 是否注册成功
     */
    public boolean registerNv21(Context context, byte[] nv21, int width, int height, FaceInfo faceInfo, String name, String registerName) {
        synchronized (this) {
             ...
             */
            File featureDir = new File(getAppRootDir(), "temp_features");
            Log.e("TAG", "特征值存储路径featureDir=========" + featureDir.getAbsolutePath());
             ...
             */
            File imgDir = new File(getAppRootDir(), "imgs");
            if (!imgDir.exists()) {
